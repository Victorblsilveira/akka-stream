...
object AdvancedBackpressure extends App {
  ...
  val events = List(
    ...
    PagerEvent("Number og HTTP 500 spiked", new Date),
    PagerEvent("A service stopped responding", new Date),
  )

  val eventSource = Source(events)
  val onCallEngineer = "daniel@rockthejv,.com"
  ...
  */
  ...
  // Produces other elements of same type - On demand
  ...
  // Works same as extrapolator but not on demand, create all time
