...
//    case Failure(ex) => println(s"The sum of the elements did not be computed: $ex")
//  }
...
//  }
  ...
  /**
   *  - return the las element out of a source (Use Sink.last)
   *  - compute total word count pout of a stream of sentences
   *     - map, fold, reduce
   */
